...
class Replica {
  constructor(site, dataDir, { now, failWrites = 0, legacy = false } = {}) {
    this.site = site;
    this.legacy = legacy;
  ...
  async init() {
    ...
    this.engine = new Engine(this.site, { now: this.now, legacy: this.legacy });
    if (loaded) {
  ...
  since(vclock, opts) {
    return this.engine.opsSince(vclock, opts);
  }
