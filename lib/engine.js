...
const KNOWN_CAPABILITY_KEYS = new Set(Object.keys(CAPABILITIES));

// 同步信封上允许出现的非业务字段（其余顶层字段按普通数据处理；__ 前缀的操作字段受保护）
const KNOWN_OP_MARKERS = new Set(["__invalid", "__invalidReason"]);

// 同步入口顶层信封的严格白名单（由 lib/sync.js 的 validateEnvelope 使用）。
// 白名单之外的顶层键——尤其是任何 __ 协议字段——一律视为无法识别的协议数据，整包拒绝。
const ENVELOPE_SCHEMAS = Object.freeze({
  pull: new Set(["site", "vclock", "capabilities"]),
  push: new Set(["site", "ops", "capabilities"]),
  exchange: new Set(["peer", "url"])
});

// 校验同步入口的顶层信封：未知顶层字段 / 未知能力键 -> 抛 EngineError。
// 必须在任何状态读取或写入之前调用（fail-closed）。
function validateEnvelope(kind, body) {
  const schema = ENVELOPE_SCHEMAS[kind];
  if (!schema) throw new EngineError(`未知同步入口：${kind}`, 400);
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    throw new EngineError("同步信封必须是 JSON 对象", 400);
  }
  for (const key of Object.keys(body)) {
    if (!schema.has(key)) {
      throw new EngineError(`无法识别的同步协议字段：${key}（入口 ${kind}），整包拒绝`, 409);
    }
  }
  if (body.capabilities !== undefined) {
    if (!body.capabilities || typeof body.capabilities !== "object" || Array.isArray(body.capabilities)) {
      throw new EngineError("capabilities 必须是对象", 409);
    }
    for (const key of Object.keys(body.capabilities)) {
      if (!KNOWN_CAPABILITY_KEYS.has(key)) {
        throw new EngineError(`无法识别的能力声明：capabilities.${key}，整包拒绝`, 409);
      }
    }
  }
}

// 递归检查操作负载内任何层级的未知 __ 协议字段（操作顶层由 _validateProtocolMarkers 处理）。
function assertNoUnknownMarkersDeep(value, opId, path) {
  if (Array.isArray(value)) {
    value.forEach((item, i) => assertNoUnknownMarkersDeep(item, opId, `${path}[${i}]`));
  } else if (value && typeof value === "object") {
    for (const [key, child] of Object.entries(value)) {
      if (key.startsWith("__")) {
        throw new EngineError(`操作 ${opId} 在 ${path}.${key} 携带无法识别的协议字段，整包拒绝`, 409);
      }
      assertNoUnknownMarkersDeep(child, opId, `${path}.${key}`);
    }
  }
}
...
class Engine {
  ...
  _validateProtocolMarkers(op) {
    ...
    // 负载内任何层级出现未知 __ 协议字段同样 fail-closed（墓碑标记只允许出现在操作顶层）。
    assertNoUnknownMarkersDeep(op.payload, op.opId, "payload");
  }
